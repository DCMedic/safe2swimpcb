...
- `flag_observations_auto.csv`: future automated Know the Gulf observations. One snapshot is retained per day plus each detected status change. This is also a primary evidence tier. Historical rows may retain legacy Safe2Swim source labels when that was the collector identity at the time.
...
- `nws_flag_overlap_audit.csv`: date-by-date comparison of recovered NWS Bay flags against primary PCBFLAGS/automatic observations on overlapping dates.
...
- `westendpcb_flag_overlap_audit.csv`: comparison of later West End PCB posts against dates already represented by primary or NWS-recovered evidence.
...
Primary PCBFLAGS/automatic records have reliable Purple reporting. Recovered NWS records retain Purple when explicitly reported, but an NWS record that does not mention Purple is marked `purple_known=false`; absence of a Purple mention is not interpreted as evidence that no Purple flag was posted. Purple frequencies therefore use only records with reliable Purple status.

## Brand-migration provenance

The project was rebranded from Safe2Swim PCB to **Know the Gulf** in August 2026. Historical records are not retroactively relabeled when the old collector identity is part of the evidence trail. Product-facing metadata, current documentation, and future collector identifiers use Know the Gulf.
