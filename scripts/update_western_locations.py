...
IEM_RETRIEVE='https://mesonet.agron.iastate.edu/cgi-bin/afos/retrieve.py';IEM_VIEW='https://mesonet.agron.iastate.edu/p.php?pid={}';PIL='SRFMOB';DEFAULT_START=date(2008,1,1);NOAA_STATION='8729511'
LOCATIONS={'destin':{'lat':30.3935,'lon':-86.4958,'official':'https://www.destinfire.gov/today-s-warning-condition-beach-flags'},'okaloosa-island':{'lat':30.39136,'lon':-86.59319,'official':'https://www.myokaloosa.com/ps/beach-safety'}}
...
def tides(a,b):
    ...
    while cur<=end:
        stop=min(pd.Timestamp(f'{cur.year}-12-31'),end);data=get_json('https://api.tidesandcurrents.noaa.gov/api/prod/datagetter',{'begin_date':cur.strftime('%Y%m%d'),'end_date':stop.strftime('%Y%m%d'),'station':NOAA_STATION,'product':'predictions','datum':'MLLW','time_zone':'lst_ldt','interval':'hilo','units':'english','application':'KnowTheGulf','format':'json'},timeout=90);p=pd.DataFrame(data.get('predictions',[]))
        if not p.empty:
...
def environment(outdir,cfg,daily):
    ...
    if need:
        a,b=min(need),max(need);w=weather(cfg['lat'],cfg['lon'],a,b);m=marine(cfg['lat'],cfg['lon'],a,b);t=tides(a,b);n=pd.DataFrame({'date':pd.date_range(a,b).strftime('%Y-%m-%d')}).merge(w,on='date',how='left').merge(m,on='date',how='left').merge(t,on='date',how='left');n=n[n.date.isin(set(need))];n['data_quality']='finalized';n['weather_source']='Open-Meteo ERA5';n['marine_source']='Open-Meteo ERA5-Ocean';n['tide_source']=f'NOAA CO-OPS {NOAA_STATION} East Pass predictions';n['updated_at']=now_local().isoformat();old=n if old.empty else pd.concat([old[~old.date.astype(str).isin(set(need))],n],ignore_index=True);old.sort_values('date').to_csv(p,index=False)
...
def current_destin(outdir):
    u=LOCATIONS['destin']['official'];r=session().get(u,timeout=30);r.raise_for_status();flag=parse_destin_flag(r.text);now=now_local();(outdir/'current_flag.json').write_text(json.dumps({'flag':flag,'label':flag,'severity':FLAG_SEVERITY[flag],'last_verified_at':now.isoformat(),'source_name':'Destin Fire Control District / Destin Beach Safety','source_url':u,'method':'Scheduled public current-condition snapshot','stale_after_hours':2},indent=2)+'\n')
def summarize(outdir,daily,env):
    merged=daily.merge(env,on='date',how='left') if not env.empty else daily.copy();merged.to_csv(outdir/'history_daily.csv',index=False);merged.to_json(outdir/'history_daily.json',orient='records');n=len(daily);high=int((daily.rip_current_risk=='High').sum());mod=int(daily.rip_current_risk.isin(['Moderate','High']).sum());(outdir/'summary.json').write_text(json.dumps({'start':str(daily.date.min()),'end':str(daily.date.max()),'days_with_data':n,'high_rip_days':high,'high_rip_pct':round(100*high/n,1) if n else None,'moderate_or_worse_days':mod,'moderate_or_worse_pct':round(100*mod/n,1) if n else None,'source':'NWS Mobile/Pensacola SRFMOB + Open-Meteo ERA5/ERA5-Ocean + NOAA CO-OPS East Pass','updated_at':now_local().isoformat()},indent=2)+'\n')
def main():
    shared=DATA/'western';shared.mkdir(parents=True,exist_ok=True);daily=srf_history(shared)
    for slug,cfg in LOCATIONS.items():
        out=DATA/slug;out.mkdir(parents=True,exist_ok=True);daily.to_csv(out/'nws_srf_daily.csv',index=False);env=environment(out,cfg,daily);summarize(out,daily,env)
        if slug=='destin':current_destin(out)
        else:(out/'current_flag.json').write_text(json.dumps({'flag':None,'label':'Check official county flag','last_verified_at':None,'source_name':'Okaloosa County Beach Safety','source_url':cfg['official'],'method':'Official daily flag is published via county Facebook/text alert; no stable machine-readable county webpage is currently available.','stale_after_hours':0},indent=2)+'\n')
