...
def extract_explicit_flag_from_json(obj: object, beach_names: list[str], require_beach=True) -> tuple[str | None, str | None]:
    ...
    # Scope extraction to the smallest structured record that contains the
    # target beach. This prevents another beach's flag in the same API payload
    # from being selected merely because the document mentions our beach too.
...
def extract_explicit_flag(html: str, beach_names: list[str]) -> tuple[str | None, str | None]:
    ...
    # Plain rendered text is accepted only when it explicitly says the current
    # or today's flag/status. Generic hazard-key/legend text is never evidence.
